...
import static input.InputReader.getStringList;
...
public class BinaryBoarding {
    ...
    public BinaryBoarding(String filename) {
        this.directions = getStringList(filename);
    }
    ...
    private int getPosition(String[] directions, int end, String leftMarker) {
        int start = 0;
        for (String direction : directions) {
            if (direction.equals(leftMarker)) {
                end = ((start + end) / 2);
            } else {
                start = ((start + end) / 2) + 1;
            }
